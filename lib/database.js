...
class DatabaseManager {
  constructor() {
    const dbPath = path.resolve(process.cwd(), "./data/datenbank.db")
...
`)
  ...
  incrementPendingMediaCount(waChatId, count, ttlMinutes = 10) {
    try {
      ...
      INSERT OR REPLACE INTO pending_media_counts (wa_chat_id, count, expires_at)
      VALUES (?, COALESCE((SELECT count FROM pending_media_counts WHERE wa_chat_id = ?), 0) + ?, ?)
    `)
      stmt.run(waChatId, waChatId, count, expiresAt)
  ...
  executeInTransaction(operation) {
    ...
    try {
      ...
      console.error(`${getTimestamp()} | Database transaction error:\n`, error)
      throw error
    }
  }

  /**
   * Improved chat mapping creation with transaction safety
   */
  createChatMapping(waChatId, dcChannelId, chatName, chatType = "contact") {
    return this.executeInTransaction(() => {
      // Check if mapping already exists
      const existing = this.getChatMapping(waChatId)

      if (existing) {
        // Update existing mapping
        const stmt = this.db.prepare(`
        UPDATE chat_mappings 
        SET dc_channel_id = ?, chat_name = ?, chat_type = ?, last_activity = CURRENT_TIMESTAMP 
        WHERE wa_chat_id = ?
      `)
        return stmt.run(dcChannelId, chatName, chatType, waChatId)
      } else {
        // Create new mapping
        const stmt = this.db.prepare(`
