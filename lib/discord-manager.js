    ...
    try {
      ...
      if (url.includes("tenor.com")) {
        ...
        if (url.includes("/view/")) {
          // Extract ID from view URL: https://tenor.com/view/example-12345678
          const match = url.match(/\/view\/[^-]+-(\d+)/)
          if (match) {
            const gifId = match[1]
            directUrl = `https://media.tenor.com/images/${gifId}/tenor.gif`
          }
        } else if (url.includes("/gif/")) {
          // Extract ID from gif URL: https://tenor.com/gif/example-12345678
      ...
      // Handle Giphy URLs
        ...
        if (url.includes("/gifs/")) {
          // https://giphy.com/gifs/example-abc123def456
          const match = url.match(/\/gifs\/[^-]+-([a-zA-Z0-9]+)/)
          if (match) gifId = match[1]
        } else if (url.includes("/gif/")) {
          // https://giphy.com/gif/example-abc123def456
          const match = url.match(/\/gif\/[^-]+-([a-zA-Z0-9]+)/)
          if (match) gifId = match[1]
        } else {
          // Try to extract any alphanumeric ID
          const match = url.match(/([a-zA-Z0-9]{10,})/)
          if (match) gifId = match[1]
        }

        if (gifId) {
          directUrl = `https://media.giphy.com/media/${gifId}/giphy.gif`
        }
      }

      console.log(`${getTimestamp()} | 🔗 Direct URL: ${directUrl}`)
      ...
      console.log(`${getTimestamp()} | ✅ Downloaded GIF: ${filename} (${stats.size} bytes)`)
  ...
  async handleGifUrls(message, waChatId, gifUrls) {
    for (const url of gifUrls) {
      try {
        ...
        if (gifData) {
          // Record the message to prevent echo
          if (this.database) {
            this.database.recordDiscordSentMessage(waChatId, url)
          }

          // Send as video to WhatsApp (GIFs work better as videos)
          await this.whatsappManager.sendVideo(waChatId, gifData.filePath, gifData.filename, `GIF: ${url}`)
