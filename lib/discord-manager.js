...
class DiscordManager {
  ...
  async downloadGifFromUrl(url) {
    try {
      ...
      // For Tenor URLs, try to get the direct GIF URL
      if (url.includes("tenor.com")) {
        // Extract the GIF ID and construct direct URL
        const match = url.match(/\/(\d+)/)
        if (match) {
          const gifId = match[1]
          url = `https://media.tenor.com/images/${gifId}/tenor.gif`
        }
      }

      // For Giphy URLs, try to get the direct GIF URL
      ...
      console.log(`${getTimestamp()} | Downloaded GIF from URL: ${url}`)
  ...
  extractGifUrls(content) {
    const gifUrlRegex = /(https?:\/\/(?:tenor\.com|giphy\.com)\/[^\s]+)/gi
    return content.match(gifUrlRegex) || []
  }
    ...
    for (const url of gifUrls) {
      try {
        ...
        if (gifData) {
          ...
          await this.whatsappManager.sendVideo(waChatId, gifData.filePath, gifData.filename, message.content)

          // Clean up temp file
          if (this.mediaManager) {
            await this.mediaManager.cleanupTempFile(gifData.filePath)
          }

          await message.react("✅")
          console.log(`${getTimestamp()} | ✅ Sent GIF from URL to WhatsApp`)
        } else {
          await message.react("❌")
        }
      } catch (error) {
        console.error(`${getTimestamp()} | Error handling GIF URL:\n`, error)
        await message.react("❌")
      }
    }
  }
  ...
  async handleQuotedMessage(message, waChatId) {
    try {
      ...
      if (referencedMessage) {
        ...
        const quotedContent = referencedMessage.content || "[Media/Attachment]"
        const quotedAuthor = referencedMessage.author.username
        ...
        // Format the quoted message for WhatsApp
        const quotedText = `> ${quotedAuthor}: ${quotedContent}\n\n${message.content}`
        return quotedText
      }
    } catch (error) {
  ...
  async handleDiscordMediaMessage(message, waChatId) {
    ...
    for (const attachment of attachments) {
      ...
      try {
        ...
        // Create a unique identifier for this media message
        // Use a format that will match what we create in WhatsApp manager
        const mediaIdentifier = `DISCORD_MEDIA:${attachment.name}:${attachment.size}`
        if (this.database) {
          this.database.recordDiscordSentMessage(waChatId, mediaIdentifier, 10) // 10 minute TTL for media
        }

        // Also record the caption if present
  ...
  async sendMediaToChannel(channelId, mediaAttachment, messageData) {
    ...
    try {
      ...
      if (mediaAttachment.caption) {
        embed.description = `**Caption:** "${mediaAttachment.caption}"`
      }
  ...
  createMessageEmbed(message, isOwnMessage = false) {
    ...
    switch (type) {
      ...
      case "text":
        ...
        embed.description = messageText ? `"${messageText}"` : "*No text content*"
        break

      case "image":
        embed.title = `${this.getMessageTypeEmoji(type)} Image`
        if (messageText) {
          embed.description = `**Caption:** "${messageText}"`
        }
        break
      ...
      case "document":
        ...
        if (messageText) {
          embed.description = `**Filename:** "${messageText}"`
        }
        break
  ...
  getHelpText(platform = "discord") {
    ...
    help += "💬 Message quoting support" + nl + nl
    ...
    help += "• Media files are processed and forwarded with captions" + nl + nl
    ...
    if (platform === "discord") {
      help += b("📤 SEND COMMAND EXAMPLES") + nl
      help += c(`${this.commandPrefix}send 1234567890 Hello!`) + " - Send to contact by phone number" + nl
      help += c(`${this.commandPrefix}send 1234567890@c.us Hello!`) + " - Send to contact by chat ID" + nl
      help += c(`${this.commandPrefix}send 1234567890@g.us Hello everyone!`) + " - Send to group by chat ID" + nl + nl
    }

    if (platform === "discord") {
      help += nl + "Need more help? Check the bridge logs or open an issue on GitHub\nhttps://github.com/w0lfzk1n."
    }
