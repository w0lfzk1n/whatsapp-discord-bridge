...
class MediaManager {
    ...
    try {
      return new Promise((resolve) => {
        ffmpeg.getAvailableFormats((err, formats) => {
          if (err) {
            console.warn(`${getTimestamp()} | ⚠️ FFmpeg not available: ${err.message}`)
            console.warn(`${getTimestamp()} | GIF to MP4 conversion will not be available`)
            this.ffmpegAvailable = false
            resolve(false)
          } else {
            console.log(`${getTimestamp()} | ✅ FFmpeg is available for GIF conversion`)
      ...
      console.error(`${getTimestamp()} | Error checking FFmpeg availability:`, error)
      this.ffmpegAvailable = false
      return false
    }
  }
  ...
  /**
   * Converts a GIF file to MP4 format for WhatsApp compatibility
  ...
  async convertGifToMp4(gifPath) {
    if (!this.ffmpegAvailable) {
      throw new Error("FFmpeg is not available for GIF conversion")
    }
    ...
    try {
      ...
      console.log(`${getTimestamp()} | 🎬 Converting GIF to MP4: ${path.basename(gifPath)} (${stats.size} bytes)`)

      // Create output path in the temp directory
      const timestamp = Date.now()
      const mp4Filename = `gif_${timestamp}.mp4`
        ...
        try {
          ffmpeg(gifPath)
            .outputOptions([
              ...
              "-vf scale=trunc(iw/2)*2:trunc(ih/2)*2", // Ensure dimensions are even (required for some codecs)
              "-movflags faststart", // Optimize for web playback
              "-vcodec libx264", // Use H.264 codec
              "-preset fast", // Encoding preset (balance between speed and quality)
              "-crf 23", // Quality level (lower is better, 23 is a good balance)
              "-f mp4", // Force MP4 format
            ])
            .noAudio() // GIFs don't have audio
            .on("start", (commandLine) => {
              console.log(`${getTimestamp()} | 🎬 FFmpeg command: ${commandLine}`)
            })
            .on("progress", (progress) => {
              if (progress && progress.percent) {
                console.log(`${getTimestamp()} | 🎬 Conversion progress: ${Math.round(progress.percent)}%`)
              }
            })
            .on("end", () => {
              console.log(`${getTimestamp()} | ✅ GIF successfully converted to MP4: ${mp4Path}`)
              resolve(mp4Path)
            })
            .on("error", (err) => {
              console.error(`${getTimestamp()} | ❌ Error converting GIF to MP4:`, err)
              reject(err)
            })
            .save(mp4Path)
        } catch (error) {
          console.error(`${getTimestamp()} | ❌ Error in FFmpeg execution:`, error)
          reject(error)
        }
      })
    } catch (error) {
      console.error(`${getTimestamp()} | ❌ Error in GIF conversion:`, error)
      throw error
    }
  }
